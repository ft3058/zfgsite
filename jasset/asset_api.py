...
def group1_add_asset(group1, asset_id=None, asset_ip=None):
    """
    资产组添加资产
    Asset group add a asset
    """
...
def db_add_group1(**kwargs):
    """
    add a asset group in database
    数据库中添加资产
    """
    name = kwargs.get('name')
    group1 = get_object(AssetGroup1, name=name)
    asset_id_list = kwargs.pop('asset_select')

    if not group1:
        group1 = AssetGroup1(**kwargs)
        group1.save()
        for asset_id in asset_id_list:
            group1_add_asset(group1, asset_id)
