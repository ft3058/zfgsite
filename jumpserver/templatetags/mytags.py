...
from jasset.models import AssetGroup1
...
@register.filter(name='rule_member_join')
def rule_member_join(instance, member):
    """
    instance is a rule object,
    use to get the number of the members
    :return:
    ...
    # counts = member.all()  # .count()
    mems = member.all()
    return ','.join([obj.name for obj in mems])


@register.filter(name='rule_domain_member_count')
def rule_domain_member_count(instance, member):
    """
    instance is a rule object,
    use to get the number of the members
    :return:
...
@register.filter(name='rule_domain_member_join')
def rule_domain_member_join(instance, member):
    """
    instance is a rule object,
    use to get the number of the members
...
@register.filter(name='group1_str2')
def group1_str2(group_list):
    """
    将用户组列表转换为str
    """
    if len(group_list) < 3:
        return ' '.join([group1.name for group1 in group_list])
    else:
        return '%s ...' % ' '.join([group1.name for group1 in group_list[0:2]])
