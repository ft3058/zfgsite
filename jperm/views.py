...
from jasset.models import Asset, AssetGroup, Domains
from jperm.models import PermRole, PermRule, PermSudo, PermPush, PermRuleDomain
...
@require_role('admin')
def perm_rule_list_domains(request):
    """
    list rule page
    授权规则列表
    """
    # 渲染数据
    header_title, path1, path2 = "授权规则", "规则管理", "查看规则(域)"
    # 获取所有规则
    rules_list = PermRuleDomain.objects.all()
    rule_id = request.GET.get('id')
    # TODO: 搜索和分页
    keyword = request.GET.get('search', '')
    if rule_id:
    ...
    # return my_render('jperm/perm_rule_list.html', locals(), request)
    return my_render('jperm/perm_rule_list_domains.html', locals(), request)
...
@require_role('admin')
def perm_rule_domains_detail(request):
    """
    rule detail page
    授权详情
    """
    ...
    try:
        if request.method == "GET":
            ...
            if not rule_id:
                raise ServerError("Rule Detail - no rule id get")
            rule_obj = PermRuleDomain.objects.get(id=rule_id)

            user_obj = rule_obj.user.all()
            domains_obj = rule_obj.domains.all()
            # user_group_obj = rule_obj.user_group.all()
            # asset_obj = rule_obj.asset.all()
            # asset_group_obj = rule_obj.asset_group.all()
            # roles_name = [role.name for role in rule_obj.role.all()]
            ...
            rule = rule_obj
            users = user_obj
            domains_list = domains_obj
            print 'len domains_obj: ', len(domains_obj)
            # user_groups = user_group_obj
            # assets = asset_obj
            # asset_groups = asset_group_obj
    except ServerError, e:
    ...
    return my_render('jperm/perm_rule_domains_detail.html', locals(), request)
...
def perm_rule_domains_add(request):
    """
    add rule page
    添加授权 for domains
    """
    # 渲染数据
    header_title, path1, path2 = "授权规则", "规则管理", "添加规则(域)"
    ...
    if request.method == 'POST':
        # 获取用户选择的 用户,用户组,资产,资产组,用户角色
        users_select = request.POST.getlist('user', [])  # 需要授权用户
        domains_select = request.POST.getlist('domains', [])  # 需要授权用户
        # user_groups_select = request.POST.getlist('user_group', [])  # 需要授权用户组
        # assets_select = request.POST.getlist('asset', [])  # 需要授权资产
        # asset_groups_select = request.POST.getlist('asset_group', [])  # 需要授权资产组
        # roles_select = request.POST.getlist('role', [])  # 需要授权角色
        ...
        try:
            rule = get_object(PermRuleDomain, name=rule_name)
            # dm_rule = PermRuleDomain.objects.filter()

            if rule:
                raise ServerError(u'授权规则 %s 已存在' % rule_name)

            if not rule_name:
                raise ServerError(u'规则名称不能为空')

            if not users_select:
                raise ServerError(u'用户名称不能为空')

            if not domains_select:
                raise ServerError(u'模块名称不能为空')

            # 获取需要授权的用户列表
            users_obj = [User.objects.get(id=user_id) for user_id in users_select]
            domains_obj = [Domains.objects.get(id=dm_id) for dm_id in domains_select]

            # 仅授权成功的，写回数据库(授权规则,用户,用户组,资产,资产组,用户角色)
            rule = PermRuleDomain(name=rule_name, comment=rule_comment)
            rule.save()
            rule.user = users_obj
            # rule.role = roles_obj
            rule.domains = domains_obj
            rule.save()

            msg = u"成功添加(域)授权规则：%s" % rule.name
            return HttpResponseRedirect(reverse('rule_list_domains'))
        except ServerError, e:
            error = e
    return my_render('jperm/perm_rule_domains_add.html', locals(), request)
...
@require_role('admin')
def perm_rule_domains_edit(request):
    """
    edit rule page
    """
    # 渲染数据
    ...
    return my_render('jperm/perm_rule_domains_edit.html', locals(), request)
...
@require_role('admin')
def perm_rule_domains_delete(request):
    """
    use to delete rule
    """
    if request.method == 'POST':
        ...
        rule_obj = PermRuleDomain.objects.get(id=rule_id)
        rule_obj.delete()
        return HttpResponse(u"删除授权(域)规则：%s" % rule_obj.name)
    else:
        return HttpResponse(u"不支持该操作")
